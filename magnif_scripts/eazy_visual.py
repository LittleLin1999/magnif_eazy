...
if __name__ == "__main__":
    ...
    field_info = { 'm0416': {'eazy_hdf5': 'MAGNIF_M0416_CIRC1_v2.0.eazy.h5',
                             'img_path': '/data_150T/public/MAGNIF/images/',
                             'phot_cat': '/data_150T/public/MAGNIF/catalog/m0416_merged_phot_v2.0.ecsv',
                             'save_dir': 'MAGNIF_photoz_202405/MAGNIF_MACS0416_CIRC1_v2p0_EAZY',
                                'save_suffix': 'MAGNIF_MACS0416_v2p0'},
                    'a370': {'eazy_hdf5': 'MAGNIF_A370_CIRC1_v2.1.eazy.h5',
                                'img_path': '/data_150T/public/MAGNIF/images/',
                                'phot_cat': '/data_150T/public/MAGNIF/catalog/a370_merged_phot_v2.1.ecsv',
                                'save_dir': 'MAGNIF_photoz_202405/MAGNIF_A370_CIRC1_v2p1_EAZY',
                                'save_suffix': 'MAGNIF_A370_v2p1'},
                    'a2744': {'eazy_hdf5': 'MAGNIF_A2744_CIRC1_v1.0.eazy.h5',
                                'img_path': '/data_150T/public/MAGNIF/images/',
                                'phot_cat': '/data_150T/public/MAGNIF/catalog/a2744_merged_phot_v1.0.ecsv',
                                'save_dir': 'MAGNIF_photoz_202405/MAGNIF_A2744_CIRC1_v1p0_EAZY',
                                'save_suffix': 'MAGNIF_A2744_v1p0'},
                    'm1149': {'eazy_hdf5': 'MAGNIF_M1149_CIRC1_v1.0.eazy.h5',
                                'img_path': '/data_150T/public/MAGNIF/images/',
                                'phot_cat': '/data_150T/public/MAGNIF/catalog/m1149_merged_phot_v1.0.ecsv',
                                'save_dir': 'MAGNIF_photoz_202405/MAGNIF_M1149_CIRC1_v1p0_EAZY',
                                'save_suffix': 'MAGNIF_M1149_v1p0'}
                             }


    field = args.field.lower()
    eazy_hdf5 = field_info[field]['eazy_hdf5']
    img_path = field_info[field]['img_path']
    phot_cat = field_info[field]['phot_cat']
    save_dir = field_info[field]['save_dir']
    save_suffix = field_info[field]['save_suffix']
    ...
    # load photometry
    phot = Table.read(phot_cat, format='ascii.ecsv')
    ...
    for colname in phot.colnames:
        if colname.endswith('_CIRC1'):
            ...
            tmp_img = img_path + f'{field.lower()}_{filt_tmp.lower()}_drz.fits'
            if len(glob.glob(tmp_img)) == 0:
                tmp_img = img_path + f'{field.upper()}_{filt_tmp.upper()}_drz.fits'
            if len(glob.glob(tmp_img)) == 0:
                tmp_img = img_path + f'{field.lower()}_{filt_tmp.lower()}.fits'
            image_list[filt_tmp] = glob.glob(tmp_img)[0] if len(glob.glob(tmp_img)) else None
    print(image_list)
    ...
    for filt in image_list:
        ...
        # except IndexError:
        #     img_data = fits.open(image_list[filt])[0].data
        #     img_header = fits.open(image_list[filt])[0].header
        #     image_dict[filt] = NDData(data=img_data, wcs=WCS(img_header))
        except:
            print(f'{filt} failed')
    ...
    show_ez = partial(show_eazy_result, fieldname=field.upper(), save_dir=save_dir, save_suffix=save_suffix,phot_ver=eazy_hdf5.split('_')[-1].split('eazy')[0], 
    phot_type=eazy_hdf5.split('_v')[0].split('_')[-1])
